...
@export var bullet_swap_target: BULLET_TYPE
...
func additional_ready_steps() -> void:
	self.get_material().set_shader_parameter("outline_color", bullet_color_dict[bullet_swap_target])
	
func collected(body:Node2D) -> void:
	BulletBus.bullet_swap.emit(bullet_swap_target)
